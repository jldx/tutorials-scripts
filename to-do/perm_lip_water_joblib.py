...
if args.suffixname:
    ...
    suf=""
PERM_FILE="permeation{}.joblib".format(suf)
...
waterox_tot=u.select_atoms("name OW")
...
for (isub,waterox) in enumerate(list_ag):
    ...
    x = np.zeros((nb_waters,nframes))
    y = np.zeros((nb_waters,nframes))
    ...
    for iframe,ts in enumerate(u.trajectory[1:]):
        ...
        #coordinates
        x[:,iframe]=waterox.positions[:,0]
        y[:,iframe]=waterox.positions[:,1]
    ...
    for index,value in enumerate(traj_simp[:,:]):
        ...
        if events.size:
            for i,time_frame in enumerate(events):
                ...
                resindex=waterox.ids[index]
                end_frame = abs(time_frame)
                start_frame = end_frame - duration_frames[i]
                posx = x[index][start_frame:end_frame]
                posy = y[index][start_frame:end_frame]
                posz = z[index][start_frame:end_frame]
                
                results.append([isub, index, resindex, resid, PS_PER_FRAME, start_frame, end_frame, duration_frames[i], direction_perm[i], posx, posy, posz])

results = pd.DataFrame(results, columns=['isub', 'isub_index', 'resindex', 'resid', 'PS_PER_FRAME', 'start_frame', 'end_frame', 'duration_nframe', 'direction', 'permx', 'permy', 'permz'])
joblib.dump(results, PATH_TO_OUTPUT+PERM_FILE)
